...
class PlayingCardView: UIView {
    ...
    var card = SetCard(color: .type1, shape: .type3, fill: .type3, numberOfShapes: .type3) { didSet { setNeedsDisplay(); setNeedsLayout() } }
    
    /// A card face background color.
    @IBInspectable
    var faceCardBackgroundColor: UIColor = #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0) {
        didSet { setNeedsDisplay() }
    }
    
    /// Showes whether a card is face up or not.
    @IBInspectable
    var isFaceUp: Bool = true { didSet { setNeedsDisplay(); setNeedsLayout() } }
    
    /// Showes whether a card is selected or not at that moment.
    @IBInspectable
    var isSelected: Bool = false { didSet { setNeedsDisplay(); setNeedsLayout() } }
    
    /// Showes whether a card is matched or not at that moment.
    ...
    private func drawType2Shape(in rect: CGRect) -> UIBezierPath {
        ...
        path.addCurve(
            to: CGPoint(x: rect.minX + rect.width * 4/5, y: rect.minY + rect.height / 8), controlPoint1: CGPoint(x: rect.minX + rect.width * 1/2 + sqdx, y: rect.minY + rect.height / 8 + sqdy), controlPoint2: CGPoint(x: rect.minX + rect.width * 4/5 - sqdx, y: rect.minY + rect.height / 8 + sqdy))
    ...
    private func configureState() {
        backgroundColor = isFaceUp ? #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0) : #colorLiteral(red: 0, green: 0.4784313725, blue: 1, alpha: 1)
        isOpaque = false
        contentMode = .redraw
    ...
    /// Mekes a full copy of a self card including its position.
    ///
    /// - Returns: A copy of a self card.
    func copyCard() -> PlayingCardView {
        let copy = PlayingCardView()
        copy.card = self.card
    ...
    ///
    /// - Parameters:
    ///   - deckCenter: A position which a card will fly from.
    ///   - delay: An animation delaying.
    func animateDeal3(from deckCenter: CGPoint, delay: TimeInterval) {
    ...
    /// The function executes after a fly animation.
    var addDiscardPile : (() -> Void)?
    
    /// Animates a flight to a discard pile.
    ///
    /// - Parameters:
    ///   - discardPileCenter: A center of a iscard pile.
    ///   - delay: A time interval is passing before this fly animation.
