...
struct Set {
            ...
            for shape in Type.allCases {
                for fill in Type.allCases {
                    for numberOfShapes in Type.allCases {
                        let card = SetCard(color: color, shape: shape, fill: fill, numberOfShapes: numberOfShapes)
                        deck += [card]
                    }
                }
            }
    ...
    func isSet(_ testCards: [SetCard]) -> Bool {
        var sum = [0, 0, 0, 0]
        _ = testCards.map {
    ...
    private mutating func replaceOrRemove3Cards() {
        if visibleCards.count == Set.startNumberOfCards, let take3Cards =  take3FromDeck() {
    ...
    var hints: [[Int]] {
        ...
        if isSet == true {
            ...
            return hints.map { Swift.Set($0) }
                .filter {$0.isDisjoint(with: Swift.Set(matchIndices)) }
                .map {Array($0)}
        }
        return hints
    }
    ...
    static let startNumberOfCards = 12
}
